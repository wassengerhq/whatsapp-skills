...
- `get_whatsapp_chat_statistics(device, groupBy=status|agent|department|contactType|day|week|month, fromDate, toDate)` — one-call counts for those fixed groupings.
- `analyze_whatsapp_chats(device, action=statistics)` — activity stats grouped by hour/day/week/month.

These work; reach for them when their grouping matches the question. For an arbitrary window or a metric they don't group by, the list-and-aggregate calls above stay more flexible.
